...
func (this *Admin) Handle() {
    ...
    this.conn.Write([]byte("\033[0;91mпользователь\033[0;97m: \033[0m"))
    ...
    // Get password
    this.conn.SetDeadline(time.Now().Add(60 * time.Second))
    this.conn.Write([]byte("\033[0;91mпароль\033[0;97m: \033[0m"))
    ...
    spinBuf := []byte{'-', '\\', '|', '/'}
    for i := 0; i < 15; i++ {
        this.conn.Write(append([]byte("\r\033[0;91mпроверив счета... \033[31m"), spinBuf[i % len(spinBuf)]))
    ...
    if loggedIn, userInfo = database.TryLogin(username, password); !loggedIn {
        this.conn.Write([]byte("\r\033[0;91mпроизошла неизвестная ошибка\r\n"))
        this.conn.Write([]byte("\033[0;91mнажмите любую клавишу для выхода. (any key)\033[0m"))
        buf := make([]byte, 1)
        this.conn.Read(buf)
        return
    }
    ...
    this.conn.Write([]byte("\033[0;91m     Хито - революция, хито - более продвинутый, чем вы когда-либо будете        \r\n"))
    ...
    for {
        ...
        var botCount int
        this.conn.Write([]byte("\033[\033[01;97m" + username + "\033[01;91m@\033[\033[01;97mHito \033[01;91m> \033[0;97m"))
    ...
    this.conn.Write([]byte("\033[0;91m                                                                                 \r\n"))                                                                                
        ...
        if userInfo.admin == 1 && cmd == "bots" {
            ...
            for k, v := range m {
                this.conn.Write([]byte(fmt.Sprintf("\033[0;97m%s:\t%d\033[0m\r\n", k, v)))
            }
            continue
        }
        if cmd[0] == '-' {
